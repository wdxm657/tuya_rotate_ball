...
typedef struct
{
     ...
     * @return 0 成功 其它 失败
     */
    OPERATE_RET (*adv_pdp_active) (UINT8_T sim_id,UINT8_T cid,TUYA_MDS_PDP_TYPE_E pdp_type,PCHAR_T apn, PCHAR_T username, PCHAR_T password);
    /**
     * @brief 蜂窝移动数据PDP去激活
     * @param simId
     * @return 0 成功 其它 失败
...
typedef struct
{
    ...
    VOID*   (*convert_str)(CONST VOID *from, INT_T from_size,
                            TUYA_CELLULAR_SMS_ENCODE_E from_chset,
                            TUYA_CELLULAR_SMS_ENCODE_E to_chset, INT_T *to_size);
}TKL_CELL_SMS_INTF_T;
...
typedef struct
{
    ...
    OPERATE_RET (*key_ioctl)(INT_T cmd,VOID *argv);
