...
/**
 * @brief   Set the auto proxy state(use vendor model message) of node.
 * @param   [in] enable  auto proxy state,if enable = 0 then enable the auto proxy,enable = 1 close auto proxy.
 * @param   [in] connect_state  current ble connect state.
 * @return  SUCCESS
 *          ERROR
 * */
OPERATE_RET tkl_mesh_vd_proxy_set(UINT8_T enable, UINT8_T connect_state);

/**
 * @brief   Solicit the node proxy.
 * @param   [in] time  the timeout of auto proxy enable.
 * @return  SUCCESS
 *          ERROR
 * */
OPERATE_RET tkl_mesh_vd_on_demand_proxy_solicit(UINT16_T time);

/**
 * @brief   When the BLE gatt connect, use this function to notify sdk.
 * @return  NULL
 * */
VOID_T tkl_mesh_vd_on_demand_proxy_connect(VOID_T);

/**
 * @brief   When the BLE gatt disconnect, use this function to notify sdk.
 * @return  NULL
 * */
VOID_T tkl_mesh_vd_on_demand_proxy_disconnect(VOID_T);
...
/**
 * @brief   Get the NID of primary netkey.
 * @return  Mesh network NID.
 * */
UINT8_T tkl_mesh_network_nid_get(VOID_T);


/**
 * @brief   Get the primary netkey of the node.
 * @param   [in] key  point of key
 * @param   [in] key_index  key index.
 * @return  NULL
 * */
VOID_T tkl_mesh_netkey_get(UINT8_T *key, UINT16_T key_index);

/**
 * @brief   Get the appkey of the node.
 * @param   [in] key  point of key
 * @param   [in] key_index  key index.
 * @return  NULL
 * */
VOID_T tkl_mesh_appkey_get(UINT8_T *key, UINT16_T key_index);
...
/**
 * @brief   Send a ble beacon, use private data format.
 * @param   [in] adv_type  adv type
 * @param   [in] data  point of data
 * @param   [in] data_len  data length
 * @return  SUCCESS
 *          ERROR
 * */
OPERATE_RET tkl_mesh_ble_private_beacon_send(TKL_BLE_GAP_ADV_TYPE_E adv_type, UINT8_T *data, UINT16_T data_len);

/**
 * @brief   Use to set the firware key sign key infor.
 * @param   [in] key  point of the key.
 * @param   [in] key_len  key lenght.
 * @param   [in] key_version  key version.
 * @return  NULL
 * */
VOID_T tkl_sign_key_version_set(UINT8_T *key, UINT8_T key_len, UINT8_T key_version);

/**
 * @brief   Use to set the firware infor to sdk which mesh dfu will use.
 * @param   [in] version  firware version.
 * @param   [in] mesh_category  mesh category.
 * @param   [in] mesh_category_ext  mesh category ext.
 * @param   [in] sign_type  firware sign type.
 * @return  NULL
 * */
VOID_T tkl_mesh_ota_infor_set(UINT16_T version, UINT16_T mesh_category, UINT16_T mesh_category_ext, UINT8_T sign_type);

/**
 * @brief   Use to set the mcu firware infor to sdk.
 * @param   [in] version  mcu firware version.
 * @param   [in] sign_type  firware sign type.
 * @return  NULL
 * */
VOID_T tkl_mesh_mcu_ota_infor_set(UINT16_T version, UINT8_T sign_type);
