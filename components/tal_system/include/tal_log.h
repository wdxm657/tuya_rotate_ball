...
#define TAL_PRINTF_CHECK(formatArg, firstVarArg)          __attribute__((format(printf, formatArg, firstVarArg)))
#else
#define TAL_PRINTF_CHECK(...)
#endif

TAL_PRINTF_CHECK(5, 6)
OPERATE_RET tal_log_module_print(CONST CHAR_T           *name,
                                 CONST TAL_LOG_LEVEL_E  level,
                                 CONST CHAR_T           *file,
                                 CONST INT32_T          line,
                                 CONST CHAR_T           *fmt,
                                ...);
TAL_PRINTF_CHECK(4, 5)
...
/**
 * @brief print a buffer in hex format with time
 *
 * @param[in] title, buffer title for print
 * @param[in] width, one line width
 * @param[in] buf, buffer address
 ...
 * @return NONE
 */
VOID tal_log_hex_dump_with_time(CONST TAL_LOG_LEVEL_E     level,
                      CONST CHAR_T              *file,
                      CONST INT_T               line,
                      CONST CHAR_T              *title,
                      UINT8_T                   width,
